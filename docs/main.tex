...
\centering
DNA sequence $x$ \, + \, cell state and optional conditions
\quad $\longrightarrow$ \quad conditioning encoder $\longrightarrow$ $\condvec$
\\[3pt]
$x$ and $\condvec$ pass through a frozen Borzoi backbone with generated LoRA updates
\quad $\longrightarrow$ \quad predicted track $\hat y$
}}
\end{center}
...
The same construction is used for convolutional layers. The generated update is reshaped
to the convolution kernel shape. During a batched forward pass, each example's effective
kernel is applied independently using a grouped convolution. Consequently, a batch may
contain different cell-state-specific kernels without running the backbone once per cell.
